<template>
  <BasePageWrapper
    titleKey="farms.edit.title"
    subtitleKey="farms.edit.subtitle"
    iconName="solar:buildings-3-outline"
  >
    <div class="farm-builder">
      <BaseLoader v-if="farmsUiFlags.isFetchingItem" />

      <template v-else>
        <section class="farm-builder__hero">
          <div class="farm-builder__hero-copy">
            <span class="farm-builder__eyebrow">{{ t('farms.edit.title') }}</span>
        ...
        <div class="farm-builder__content">
          <BaseStyledSection
            :label="t('farms.form.sections.basic_info')"
            ...
            <div class="farm-builder__grid farm-builder__grid--info">
              <div class="farm-builder__control">
                <BaseInput
                  ...
                  :error="v$.farmForm.name.$error"
                  :error-text="getFieldErrorMessage(v$.farmForm.name)"
                  required
                />
              </div>
              <div class="farm-builder__control">
                <BaseInput
                  ...
                  :error="v$.farmForm.location.$error"
                  :error-text="getFieldErrorMessage(v$.farmForm.location)"
                  required
                />
              </div>
            </div>
          ...
          <BaseStyledSection
            :label="t('farms.form.sections.palm_types')"
            :description="t('farms.form.sections.palm_types_text')"
          >
            <div class="farm-builder__stack">
              <ValidateEach
                v-for="(palm, palmIndex) in farmForm.palm_types"
                :key="palm.id || palmIndex"
                :state="palm"
                :rules="palmTypeRules"
              >
                <template #default="{ v: palmV }">
                  <article class="farm-builder__palm-card">
                    <div class="farm-builder__palm-head">
                      ...
                      <BaseButton
                        v-if="farmForm.palm_types.length > 1"
                        color="red"
                        variant="outline"
                        size="sm"
          ...
          <div class="farm-builder__actions">
            <BaseButton
              class="farm-builder__action farm-builder__action--back"
              color="gray"
              variant="outline"
              size="sm"
              @click="router.push({ name: 'farms_list' })"
            ...
            <BaseButton size="sm" :loading="farmsUiFlags.isUpdating" @click="submitForm">
...
const farmId = computed(() => route.params.id);
const { uiFlags: farmsUiFlags, record: farmRecord } = storeToRefs(farmsStore);
...
onMounted(async () => {
  await farmsStore.fetchRecord(farmId.value);
  farmForm.value = {
    name: farmRecord.value.name,
    location: farmRecord.value.location,
    palm_types: (farmRecord.value.palm_types || []).map((palmType) => ({
      id: palmType.id,
      name: palmType.name,
...
const submitForm = async () => {
  ...
  const response = await farmsStore.updateRecord(farmId.value, farmForm.value);
